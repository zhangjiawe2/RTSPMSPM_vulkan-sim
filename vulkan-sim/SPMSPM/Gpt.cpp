...
void createShaderBindingTable(
    ...
    uint32_t groupCount      = 3; // raygen, miss, hitgroup
    ...
    size_t sbtBufferSize = hitOffset + hitSize;
    ...
    // 这里选择可见设备内存，需要自己写 findMemoryType
    ...
    VkDeviceAddress sbtAddr = vkGetBufferDeviceAddress(device, &addrInfo_ray);
...
int main()
{
    ...
    loadMatrixAsRays("/home/zjw/SPMSPM/optix/RTSpMSpM/optixSpMSpM/src/data/amazon0312/amazon0312_small.mtx", hostRays,Resultrow);
    ...
    VkDeviceSize ResultBytes = sizeof(float)*5*5;
...
/* ----------10.1 创建 Descriptor Set Layout ---------- */
    ...
    descLayoutInfo.pBindings = &tlasBinding;
    ...
    vkCreateDescriptorPool(device, &poolInfo2, nullptr, &descriptorPool);
    ...
    write.dstBinding = 0; // GLSL binding = 0
    ...
    hitGroup.anyHitShader = 2;
    shader_groups.push_back(hitGroup);

    
    /* 10.5 创建Ray Tracing Pipeline*/
    ...
    createShaderBindingTable(device, rtPipeline, rtProperties, stages, sbtRegions, &rgSbt, &msSbt, &hgSbt, sbtBuffer, sbtMemory,physicalDevice);
    ...
    cout<< "*************success: SBT table binding*************"<<endl;
    /*  ----------12 Launch ----------------------*/
